...
public class OwnerShip extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().hasExtra(Constants.DRIVER_INSURANCE)){
            driverInsurance=getIntent().getStringExtra(Constants.DRIVER_INSURANCE);
        }
        if (driverInsurance!=null){
            Utils.showProgressDialog(this);
            Glide.with(this).load(Uri.parse("https://www.smartrider24.com/drvrsecdoc/"+driverInsurance)).addListener(new RequestListener<Drawable>() {
                @Override
